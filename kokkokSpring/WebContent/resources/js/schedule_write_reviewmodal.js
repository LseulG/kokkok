...
$("#scheduleReviewWriteModifyModal").on("shown.bs.modal", function(){
...
function msearchMap(){
	...
	var smapContainer = document.getElementById('MRsearchMap'), // 지도를 표시할 div 
	smapOption = {
	...
	//지도를 생성합니다    
	var smap = new daum.maps.Map(smapContainer, smapOption); 
	...
	function displayPlaces(places) {
	    ...
	    for ( var i=0; i<places.length; i++ ) {
	        ...
	        var placePosition = new daum.maps.LatLng(places[i].y, places[i].x),
	            smarker = addMarker(placePosition, i);
	            smarker.setZIndex(i);
	        ...
	        (function(smarker, title) {
	            daum.maps.event.addListener(smarker, 'mouseover', function() {
	                displayInfowindow(smarker, title);
	            });
	
	            daum.maps.event.addListener(smarker, 'mouseout', function() {
	                infowindow.close();
	            });
	...
	function addMarker(position, idx, title) {
	    var simageSrc = 'http://t1.daumcdn.net/localimg/localimages/07/mapapidoc/marker_number_blue.png', // 마커 이미지 url, 스프라이트 이미지를 씁니다
	        simageSize = new daum.maps.Size(36, 37),  // 마커 이미지의 크기
	        imgOptions =  {
	            spriteSize : new daum.maps.Size(36, 691), // 스프라이트 이미지의 크기
	            spriteOrigin : new daum.maps.Point(0, (idx*46)+10), // 스프라이트 이미지 중 사용할 영역의 좌상단 좌표
	            offset: new daum.maps.Point(13, 37) // 마커 좌표에 일치시킬 이미지 내에서의 좌표
	        },
	        smarkerImage = new daum.maps.MarkerImage(simageSrc, simageSize, imgOptions),
	            smarker = new daum.maps.Marker({
